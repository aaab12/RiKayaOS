...
int insertBlocked(int *key,pcb_t* p){
  ...
  if(ptr == NULL){
    //il semaforo con key non e presente nella ASL
...
pcb_t* removeBlocked(int *key){
  ...
  } else
  {
    ...
    //controllo se il semaforo e rimasto senza processi bloccati in coda, in questo caso lo metto nella lista dei semafori liberi
    if(scanq->next==NULL){
...
pcb_t* outBlocked(pcb_t *p){
  ...
  semd_t *ptr = getSemd(key);
  if(ptr == NULL){
    //il semaforo su cui e bloccato p non e nella ASL
    return NULL;
  } else
  { //ho trovato il semaforo a cui appartiene il pcb p (semd_t *ptr) che essendo allocato ha almeno un processo
    ...
    if(scan != p){
      ...
      //controllo se il semaforo e rimasto senza processi bloccati in coda e nel caso lo dealloco
...
pcb_t* headBlocked(int *key){
  ...
  {
    //ritorno il primo PCB bloccato in coda al semaforo. Non puo essere che non ce ne sia almeno uno o il semaforo non sarebbe nella ASL
