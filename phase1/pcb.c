...
void freePcb(pcb_t * p){
	//controllo che il PCB p non sia gia nella lista pcbFree
	list_head *scan = pcbFree->next;
	while(scan!=pcbFree){
		if(container_of(scan, pcb_t, p_next)==p){
			//p era gia nella lista pcbFree quindi non devo aggiungerlo
			return;
		} else {
			scan = scan->next;
		}
...
void insertProcQ(struct list_head *head, pcb_t *p){
  ...
  if(list_empty(head)){
	...
	{
		//la lista puntata da head non e vuota, cmp: primo PCB della lista
...
pcb_t *outProcQ(struct list_head *head, pcb_t *p){
  if(list_empty(head)){
	...
	{
		//la lista ha almeno un elemento, scanh: puntatore al campo p_next del primo PCB della lista
...
void insertChild(pcb_t *prnt, pcb_t *p){
  ...
  if(scanh->next == NULL){
	...
	{
		//prnt ha almeno un figlio, scanp = primo figlio di prnt
...
pcb_t *removeChild(pcb_t *p){
  if(p->p_child.next == NULL)
	...
	{
		//p ha almeno un figlio, scanp: primo figlio
    ...
    if(scansib->next == NULL){
      ...
      return scanp;
			...
			//scanc ha fratelli, nextp: primo fratello che diventera primo figlio
